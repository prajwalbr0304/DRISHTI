# DRISHTI Remaining Hackathon Implementation Prompts
...
4. update prompt3.md's status table only after every mandatory gate passes.
~~~
...
| 22 | Local release stabilization and functional CI/CD | **Complete (2026-07-21)** — see `docs/phase-reports/PHASE_22_REPORT.md`. Strict local release gate 16/16 mandatory PASS. |
| 23 | Live Zoho Catalyst provisioning and deployment | **Live — core DoD proven (2026-07-23)** — see `docs/phase-reports/PHASE_23_REPORT.md`. Public frontend (Slate `drishti-frvfpunc.onslate.in`, canonical) + primary API (AppSail via API Gateway) resolve to Catalyst; Auth→Gateway→AppSail→Data Store/Stratus proven (security 11/11, exact-origin CORS, readiness all-ok); six-role allow/deny matrix; **live Signal** (`prediction-requested`→`prediction_event`, `approved→queued`, retry + idempotency) and **live daily cron** (`drishti_forecast`→`cron_forecast`); **application rollback** + additive forward recovery; Stratus evidence object (versioned, sha256 MATCH, signed-URL expiry); chat session + RDS case reads. **Board/Disaster/evidence-upload now fixed + live** (report §3.8-3.10): Board CRUD; all 12 Disaster reads + overview + a forecast write (15 Data Store tables created via the reverse-engineered Console create-table API + fixture seeded); evidence file-upload→Stratus with SHA-256 verify. Data Store fixes: ZCQL≤300 pagination, datetime format, dict/list↔json round-trip. Residual non-core (report §9): pipeline Git-link+run (F1), credit snapshot (G1), delete 2 stale Slate apps (G2), the interactive Catalyst IAM login to finish the fully-automated browser E2E (E1), FIR happy-path approval, and deploying the login-UI polish (one Slate "Create Deployment"). |
| 24 | Live AWS custom-ML plane and Catalyst-to-AWS prediction proof | **Complete (2026-07-23)** — see `docs/phase-reports/PHASE_24_REPORT.md`. All 6 DoD proven live on a real **NVIDIA T4** SageMaker async endpoint (`ml.g4dn.2xlarge`, ap-south-1), then torn down. **Real TabFM on CUDA** (backend=tabfm, device=cuda, Tesla T4, weight digest `928cb350…`, peak GPU 6633 MB); **real TimesFM** (device=cuda, digest `2f776efe…`, p10/p50/p90 intervals, cutoff echoed); **Catalyst-side→AWS→Catalyst** via the real app `SignedHttpsAdapter`+`PredictionRuntime` (API Gateway→Lambda→SageMaker→validated+persisted, no browser AWS); guards (draft/evidence zero-invocation) + idempotency; fail-closed (bad digest→CUDA_UNAVAILABLE, no masquerade) + labelled CPU fallback (incontext); held-out compare TabFM 0.983 ≫ fallback 0.733 ≫ majority 0.30. Image built in AWS CodeBuild, pushed to ECR by immutable digest (`@sha256:a2944b79…`). Budget + CloudWatch alarm; endpoint deleted (`list-endpoints=[]`), ECR/S3 artifacts retained. TabFM **weights are TabFM Non-Commercial License v1.0** (research/eval only, never production); TimesFM Apache-2.0. Deployed-AppSail env wiring staged for the Console (adapter URL + Secrets Manager ref). |
| 25 | Integrated security, scale, recovery and final hackathon release | **Pending (2026-07-24)** — see `docs/phase-reports/PHASE_25_REPORT.md`. All local/security/AWS-model/load/recovery suites GREEN + local gate 16/16. Live-edge items: **RB-1/RB-4 (real Catalyst IAM login) are an owner-accepted demo deviation** — the demo uses offline role cards + gateway demo-auth (synthetic, reversible; re-enable when shortlisted). **RB-2** (live `/api/ask` 502) and **RB-3** (deployed AppSail→AWS adapter unwired; T4 torn down for cost — TabFM/TimesFM independently proven) remain open; strict acceptance stays non-zero until RB-2 closes. |
| 26 | Independent implementation completeness audit and submission package | **Audit complete — submission CONDITIONAL (2026-07-24)** — see `docs/phase-reports/PHASE_26_REPORT.md`, `FINAL_IMPLEMENTATION_AUDIT.md`, `SUBMISSION_EVIDENCE_INDEX.md`, `artifacts/phase-26/FINAL_AUDIT_MANIFEST.json`. Independent four-way audit of all 52 DoD requirements + the 26-row Catalyst matrix + 15 organizer-note requirements; architecture re-verified (route boundary 336/0; AWS endpoints=0; six-role enum parity across frontend/gateway/backend). Tally: **44 PASS, 5 PASS(offline), 1 accepted auth deviation, 2 FAIL** (RB-2 live NL 502; strict acceptance). Repaired canonical-URL drift + confirmed no mojibake/secrets. **Hackathon-demo-ready via documented deterministic fallbacks; NOT unconditionally all-live until RB-2 closes; never production-ready.** |
...
3. Update the recommended starting instruction to Prompt 18 in prompt3.md.
...
- final updated status tables in prompt3.md and, where historically appropriate,
...
2. Paste the Global Execution Contract from this file.
3. Paste only the next Pending prompt.
4. Let the agent inspect and continue existing work.
5. Complete unavoidable Zoho/AWS browser authentication if requested.
6. Do not move to the next prompt until the report exists and every Definition of
   Done item passes.

Start with Prompt 18. Prompt 26 is the final independent gate.
