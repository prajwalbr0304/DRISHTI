...
Catalyst frontend
  ...
  -> app/predict/adapter.py  (signed HTTPS client, circuit breaker)
...
The Lambda/ECS role grants only: `sagemaker:InvokeEndpoint` /
...
CloudWatch Logs. No `*` resources. See `infra/aws/adapter/iam/`.
