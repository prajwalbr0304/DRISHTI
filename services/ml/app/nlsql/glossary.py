...
GLOSSARY: list[tuple[str, str, str]] = [
    ...
    # --- Prompt 20 §A: crypto / dark-web / cross-jurisdiction (synthetic) ---
    ("cryptocurrency / crypto crime", "ಕ್ರಿಪ್ಟೋ ಅಪರಾಧ",
     "Economic & Cyber Crime head; crypto scenarios in the /scenarios registry"),
    ("dark web crime", "ಡಾರ್ಕ್‌ವೆಬ್ ಅಪರಾಧ",
     "manually classified dark-web scenarios in the /scenarios registry (no live collection)"),
    ("wallet / cryptocurrency wallet", "ಕ್ರಿಪ್ಟೋ ವಾಲೆಟ್",
     "wallet_ref entity in the scenario graph (synthetic SYN-WALLET refs)"),
    ("inter-district", "ಅಂತರ-ಜಿಲ್ಲಾ",
     "jurisdiction scope (scenario registry) / JurisdictionReassignment across districts"),
    ("inter-state", "ಅಂತರ-ರಾಜ್ಯ", "jurisdiction scope = inter_state (scenario registry)"),
    ("national", "ರಾಷ್ಟ್ರೀಯ", "jurisdiction scope = national (scenario registry)"),
    ("international / cross-border", "ಅಂತರರಾಷ್ಟ್ರೀಯ / ಗಡಿಯಾಚೆ",
     "jurisdiction scope = international/cross_border (scenario registry)"),
    ("jurisdiction referral", "ವ್ಯಾಪ್ತಿ ಉಲ್ಲೇಖ",
     "scenario.referral (inter-state/central-agency coordination)"),
    # --- gravity / status ---
