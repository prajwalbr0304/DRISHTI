...
def test_prediction_flow_and_idempotent_request(rw_rollback):
    ...
    r1 = service._create_request(conn, mv, snap["feature_snapshot_id"], "batch", "t-req", "senior_command")
    r2 = service._create_request(conn, mv, snap["feature_snapshot_id"], "batch", "t-req", "senior_command")
    assert r1["status"] == "queued"
    assert r1["prediction_request_id"] == r2["prediction_request_id"]   # idempotent
    assert r2["reused"] is True

    run = service._run_request(conn, r1["prediction_request_id"], "senior_command")
    ...
    again = service._run_request(conn, r1["prediction_request_id"], "senior_command")
...
def test_schema_mismatch_rejected(rw_rollback):
    ...
    with pytest.raises(service.SchemaMismatch):
        service._create_request(conn, mv, other_snap, "batch", "t-mismatch", "senior_command")
...
def test_unapproved_model_rejected(rw_rollback):
    ...
    with pytest.raises(service.ModelNotApproved):
        service._create_request(conn, draft_mv, snap["feature_snapshot_id"], "batch", "t-draftmv", "senior_command")
...
def test_stale_after_correction_blocks_run(rw_rollback):
    ...
    r = service._create_request(conn, mv, snap["feature_snapshot_id"], "batch", "t-stale", "senior_command")
    with pytest.raises(service.StaleSnapshot):
        service._run_request(conn, r["prediction_request_id"], "senior_command")
...
def test_review_writes_review_and_audit(rw_rollback):
    ...
    snap = builder.build_snapshot(conn, feature_schema_version_id=fsv, subject_kind="area_district",
                                  subject_ref_id=str(did), actor="t")
    r = service._create_request(conn, mv, snap["feature_snapshot_id"], "batch", "t-review", "senior_command")
    run = service._run_request(conn, r["prediction_request_id"], "senior_command")
    ...
    run2 = service._run_request(conn, r["prediction_request_id"], "senior_command")  # idempotent replay
